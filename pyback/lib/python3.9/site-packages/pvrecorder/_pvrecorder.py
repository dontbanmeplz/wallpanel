...
def default_library_path(relative: str = ""):
    ...
    if platform.system() == "Windows":
        script_path = os.path.join(
            os.path.dirname(__file__), relative, "resources", "scripts", "platform.bat"
        )
    else:
        script_path = os.path.join(
            os.path.dirname(__file__), relative, "resources", "scripts", "platform.sh"
        )
...
class PvRecorder(object):
    ...
    def __init__(
        self, frame_length: int, device_index: int = -1, buffered_frames_count: int = 50
    ):
        """
        Constructor
        ...
        status = init_func(
            frame_length, device_index, buffered_frames_count, byref(self._handle)
        )
        if status is not self.PvRecorderStatuses.SUCCESS:
            raise self._PVRECORDER_STATUS_TO_EXCEPTION[status](
                "Failed to initialize PvRecorder."
            )
    ...
    def get_available_devices() -> List[str]:
        ...
        get_available_devices_func = (
            PvRecorder._get_library().pv_recorder_get_available_devices
        )
        get_available_devices_func.argstype = [
            POINTER(c_int32),
            POINTER(POINTER(c_char_p)),
        ]
        get_available_devices_func.restype = PvRecorder.PvRecorderStatuses

        free_available_devices_func = (
            PvRecorder._get_library().pv_recorder_free_available_devices
        )
        ...
        if status is not PvRecorder.PvRecorderStatuses.SUCCESS:
            raise PvRecorder._PVRECORDER_STATUS_TO_EXCEPTION[status](
