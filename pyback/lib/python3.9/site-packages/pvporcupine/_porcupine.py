...
class Porcupine(object):
    ...
    def __init__(
        ...
        status = init_func(
            ...
            len(keyword_paths),
            (c_char_p * len(keyword_paths))(
                *[os.path.expanduser(x).encode("utf-8") for x in keyword_paths]
            ),
    ...
    def process(self, pcm: Sequence[int]) -> int:
        ...
        if len(pcm) != self.frame_length:
            raise ValueError(
                "Invalid frame length. expected %d but received %d"
                % (self.frame_length, len(pcm))
            )
        ...
        if status is not self.PicovoiceStatuses.SUCCESS:
            raise self._PICOVOICE_STATUS_TO_EXCEPTION[status](
                message="Processing failed", message_stack=self._get_error_stack()
            )
    ...
    def _get_error_stack(self) -> Sequence[str]:
        ...
        status = self._get_error_stack_func(
            byref(message_stack_ref), byref(message_stack_depth)
        )
        if status is not self.PicovoiceStatuses.SUCCESS:
            raise self._PICOVOICE_STATUS_TO_EXCEPTION[status](
